...
Folder_path = dir('./hComDraftModels');
...
for i=1:n_models
    load(['./hComDraftModels/',Folder_path{i}]) % loading the microbial model
...
for k=1:numel(abbr)
    load(['./hComDraftModels/',abbr{k},'.mat']) % loading the microbial model
    gapFilledModel = models{k};
    Model = getUnionModel(gapFilledModel,model); 
    save(['./CarvemeCommunityGapfillingMILP/',abbr{k}],'Model')
    clear model Model gapFilledModel
end

save('Results_CommunityGapFillMILP')
